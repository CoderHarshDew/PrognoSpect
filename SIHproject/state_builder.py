...
class TemporalStateBuilder:
    ...
    def prepare_timestamp(self, df):
        ...
        timestamp_col = self.find_column(
            df,
            [
                "Timestamp",
                "timestamp",
    ...
    def build_global_features(self, window):
        ...
        src_col = self.find_column(
            window,
            ["Src IP", "Source IP", "Source"]
        )

        dst_col = self.find_column(
            window,
            ["Dst IP", "Destination IP", "Destination"]
        )

        dst_port_col = self.find_column(
            window,
    ...
    def build_state(
        ...
        X_t = self.build_global_features(
            window
        )

        H_t = self.build_host_features(
            window
        )

        G_t = self.build_graph(
            window
        )

        Delta_t = self.compute_delta(
            X_t,
        ...
        # Current state ko previous bana do
    ...
    # 9. BUILD ALL TEMPORAL STATES

    def build_states(self, df):

        df, timestamp_col = (
            self.prepare_timestamp(df)
        )
